...
class WireGuardManager:
    ...
    def _normalize_awg_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Ensures legacy AmneziaWG configs are explicit when this old path is used."""
    ...
    def get_next_available_ip(self, server: Optional[Server] = None) -> str:
        ...
        if server and not self._is_local_server(server):
            ip = self.remote_manager.get_next_available_wireguard_ip(server)
            if ip:
                return ip
    ...
    def create_client_config(self, device: Device, server: Server, client_ip: str, protocol: str = "wireguard") -> str:
        """Создает конфигурацию клиента WireGuard или GRANIWG"""
        if protocol == "graniwg":
            return self.create_graniwg_client_config(device, server, client_ip)
...
"""
    ...
    def create_graniwg_client_config(self, device: Device, server: Server, client_ip: str) -> str:
...
"""
    ...
    def add_peer_to_server(self, device: Device, server: Server, client_ip: str) -> bool:
        """Добавляет пир в конфигурацию сервера"""
        # Если сервер удаленный, используем RemoteVPNManager
        if not self._is_local_server(server):
            try:
                return self.remote_manager.add_wireguard_peer(server, device, client_ip)
            except Exception as e:
