...
def get_active_subscription_details(user_id: int, db: Session) -> dict:
    """
    Возвращает детали активной подписки пользователя (или пустой dict).
    """
    try:
        ...
        plan = sub.plan
        return {
            "subscription_expires_at": sub.end_date.isoformat() if sub.end_date else None,
            "subscription_started_at": sub.start_date.isoformat() if sub.start_date else None,
            ...
            "subscription_auto_renew": sub.auto_renew,
        }
    except Exception as e:
        logger.error(f"Ошибка получения деталей подписки для пользователя {user_id}: {e}")
        return {}
