...
/// The production `/main` shell is owned by `SimpleVpnController`.
///
/// Keep the legacy service for control-plane data and old/R&D screens, but do
/// not let it independently observe and project the Android tunnel state. Two
/// state owners caused resume/startup races on Xiaomi/HyperOS.
bool get _legacyVpnRuntimeObserverEnabled => false;
...
class VpnService extends ChangeNotifier {
  ...
  // Флаг legacy refreshServers используется в server_catalog_helpers.dart.
  bool _isRefreshing = false;

  // Отдельная дедупликация control-plane snapshot. Параллельный caller должен
  // дождаться того же запроса, а не продолжить работу со старым snapshot.
  Future<void>? _controlPlaneSnapshotRefreshInFlight;
  ...
  Future<void> refreshControlPlaneSnapshot(
    ...
    final inFlight = _controlPlaneSnapshotRefreshInFlight;
    if (inFlight != null) return inFlight;
    if (!force && _lastRefreshTime != null) {
      final delta = DateTime.now().difference(_lastRefreshTime!);
      if (delta < _refreshDebounceInterval) return Future<void>.value();
    }

    _lastRefreshTime = DateTime.now();
    late final Future<void> operation;
    operation = _refreshControlPlaneSnapshotImpl(authService).whenComplete(() {
      if (identical(_controlPlaneSnapshotRefreshInFlight, operation)) {
        _controlPlaneSnapshotRefreshInFlight = null;
      }
    });
    _controlPlaneSnapshotRefreshInFlight = operation;
    return operation;
  }

  Future<void> _refreshControlPlaneSnapshotImpl(
    AuthService authService,
  ) async {
    try {
