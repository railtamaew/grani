...
# Запустите этот скрипт на сервере после загрузки проекта
...
print_status "🚀 Начинаем развёртывание GraniVPN на сервере..."
...
if [ ! -d "/opt/grani" ]; then
    ...
    print_warning "Сначала загрузите проект на сервер:"
    print_warning "  scp /tmp/granivpn-deploy-*.tar.gz root@SERVER_IP:/tmp/"
    print_warning "  ssh root@SERVER_IP"
    print_warning "  mkdir -p /opt/grani && cd /opt/grani"
    print_warning "  tar -xzf /tmp/granivpn-deploy-*.tar.gz --strip-components=1"
    exit 1
fi
...
# 13. Проверка конфигурации VPN/Redis/Celery
print_status "Проверка конфигурации (verify_vpn_deploy)..."
if docker-compose exec -T api python3 scripts/verify_vpn_deploy.py 2>/dev/null; then
    print_success "Проверка конфигурации пройдена"
else
    print_warning "Проверка конфигурации выдала предупреждения (см. выше). Для продакшена задайте CACHE_BACKEND=redis, RATE_LIMIT_STORE=redis и убедитесь, что Celery запущен."
fi
...
print_warning "   6. Мониторинг: добавьте в cron проверку /health (см. backend/docs/OPERATIONS.md, раздел 6)"
print_warning ""
...
print_status "   • Проверка здоровья: cd /opt/grani/backend && API_HEALTH_URL=http://127.0.0.1:8010/health bash scripts/check_production_health.sh"
